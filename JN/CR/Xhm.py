# -*- coding: utf-8 -*-
# 完整修复版 for XHamster (By Gemini & ChatGPT)
# 修复播放地址 403/加载失败问题，增加 m3u8 自动跳转获取 Token 逻辑
import json
import sys
...
# 兼容性导入，确保 base.spider 存在
try:
    sys.path.append('..')
    from base.spider import Spider
except ImportError:
    # 如果在非标准环境测试，提供一个基础 Spider 类防止报错
    class Spider:
...
class Spider(Spider):
    ...
    def init(self, extend=""):
        ...
        # 更新 headers 中的 origin/referer 以匹配实际 host
    ...
    def proxy(self, data, type='img'):
        ...
        try:
            if data and len(self.proxies) > 0:
                # 只有当存在代理配置时才走本地代理，否则可能返回原地址
                # 但为了稳妥，建议视频流始终走代理(如果框架支持)
